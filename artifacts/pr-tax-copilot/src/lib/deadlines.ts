  ...
  return [
    {
      ...
      penaltyNote: "Late filing/payment may incur interest, surcharges, and penalties. Verify with Hacienda/CPA.",
    ...
    },
    {
      ...
      penaltyNote: "Missing the Act 60 annual report can risk decree compliance. Verify with DDEC/attorney.",
      requiresVerification: true,
...
export function deadlinesForUser(taxYear: number, taxpayerTypes: TaxpayerType[]): Deadline[] {
  const set = new Set(taxpayerTypes);
  return seedDeadlines(taxYear).filter((d) => d.userTypes.some((t) => set.has(t)));
}
