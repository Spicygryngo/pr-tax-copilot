...
// Pure functions — no I/O — so they are trivially unit-testable (see tests/).
// Given a user's tax profile, produce the personalized required/optional checklist.
...
const PROFILE_ITEMS: ChecklistTemplate[] = [
  {
    ...
    description: "Finish the intake wizard so we can tailor your checklist, deadlines, and dashboard.",
    category: "profile",
    required: true,
  },
  {
    title: "Upload prior-year tax return",
    description: "Last year's return helps us understand carryovers, credits, and your filing pattern.",
    category: "shared",
    required: false,
  },
];
...
function makeId(seed: string): string {
  // Stable-ish id from seed for idempotent regeneration in local mode.
  let h = 0;
...
/**
 * Generate the full checklist for a profile. Templates are de-duplicated by
 * title so a contractor who is also a business owner doesn't get duplicate
 * "Upload receipts" rows.
 */
export function generateChecklist(profile: UserTaxProfile): ChecklistItem[] {
  ...
  // Conditional items driven by intake answers.
  if (profile.collectsIvu && !profile.taxpayerTypes.includes("business_owner")) {
    templates.push({
      title: "Upload IVU/SUT records",
      description: "You indicated you collect IVU/SUT. Add your collection and remittance records.",
      category: "shared",
      required: true,
    });
  }
  if (profile.hasPayroll && !profile.taxpayerTypes.includes("business_owner")) {
    templates.push({
      ...
      description: "You indicated you run payroll. Add payroll records for the filing year.",
      category: "shared",
      required: true,
    });
  }
