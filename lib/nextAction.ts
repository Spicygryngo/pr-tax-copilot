// "Next best action" prioritizer (build plan section 12E).
// Returns the single highest-priority thing the user should do next.
...
export function nextBestAction({ profile, checklist, documents }: Input): NextAction {
  ...
  // First required "upload" item that isn't done.
  const missingUpload = checklist.find(
    (c) => c.required && c.status !== "done" && /upload/i.test(c.title)
  );
  if (missingUpload) {
    return {
  ...
  // Otherwise the first required checklist item that isn't done.
